    ...
    <section ref={sectionRef} className="px-4 sm:px-6 py-12 sm:py-20">
      <div className="mx-auto max-w-7xl">
        ...
        <div className={cn("mb-12 sm:mb-16 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs uppercase tracking-[0.25em] sm:tracking-[0.35em] text-primary">Engineering</p>
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">Selected Projects</h1>
          <p className="max-w-2xl text-base sm:text-lg text-muted-foreground leading-relaxed">
            Real-world systems designed, built, and deployed end-to-end. Click any project to read the full engineering case study.
          </p>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredProjects.map((project, index) => (
            <Link
              ...
              className={cn(
                "group relative overflow-hidden rounded-xl border bg-card/40 p-6 sm:p-7 glass transition-all duration-400 active:scale-[0.99] hover-lift opacity-0 block",
            ...
            >
              ...
              <div className="mb-5 flex flex-wrap gap-2">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-md border border-border/80 bg-secondary/60 px-2.5 py-1 font-mono text-xs text-secondary-foreground transition-colors group-hover:border-primary/50 group-hover:bg-primary/10"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-2 font-mono text-xs text-muted-foreground group-hover:text-primary transition-all duration-300">
              ...
              <div className="absolute bottom-5 right-5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                Read case study
              </div>
